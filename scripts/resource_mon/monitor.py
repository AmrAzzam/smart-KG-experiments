...
# Resource monitoring script for the clients and servers except for sage.
...
def getInfo(processID):
    ...
    # Monitor CPU usage in 1 second intervals
    cpu = p.cpu_percent(interval=1)
    ram = bytes2human(p.memory_info().rss)
    # Write process info to os.csv
    write_csv_os([(datetime.now().strftime('%Y-%m-%d %H:%M:%S')), cpu, ram])
...
def getNetInfo():
    ...
    for name in nic_names:
        ...
        # Specify correct network interface here. See the output of "ip link" to get the name of the Ethernet device.
        if name == 'enp4s0':
